...
class KittiDataset(Dataset):
    ...
    def get_label(self, idx):
        ...
        for line in open(label_path, 'r'):
            ...
            occluded = int(float(line_parts[2]))  # 0=visible, 1=partly occluded, 2=fully occluded, 3=unknown
            ...
            '''
            alpha = float(line_parts[9])*math.pi/180  # object observation angle [-pi..pi]
            # xmin, ymin, xmax, ymax
            # bbox = np.array([float(line_parts[4]), float(line_parts[5]), float(line_parts[6]), float(line_parts[7])])
            # height, width, length (h, w, l)
            h, w, l = float(line_parts[4]), float(line_parts[3]), float(line_parts[2])
            # location (x,y,z) in camera coord.
            x, y, z = float(line_parts[5]), float(line_parts[6]), float(line_parts[7])
            ry = alpha  # yaw angle (around Y-axis in camera coordinates) [-pi..pi]
