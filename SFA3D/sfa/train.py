...
def train_one_epoch(train_dataloader, model, optimizer, lr_scheduler, epoch, configs, logger, tb_writer):
    ...
    progress = ProgressMeter(len(train_dataloader), [batch_time, data_time, losses],
                             prefix="Train - Epoch: [{}/{}]".format(epoch, configs.num_epochs))  #载入训练数据

    criterion = Compute_Loss(device=configs.device)  #定义损失函数
    num_iters_per_epoch = len(train_dataloader)       #每个epoch要训练数据的数量
    ...
    for batch_idx, batch_data in enumerate(tqdm(train_dataloader)):
        ...
        metadatas, imgs, targets = batch_data          #一个bactch里面的数据
        batch_size = imgs.size(0)
        # 总的举例：epoch = 1 ，global_step = batch_idx+1 ；
        # epoch = 2，len(train_dataloader)+batch_idx+1
        #epoch = 3，len(train_dataloader)*2+batch_idx+1
        global_step = num_iters_per_epoch * (epoch - 1) + batch_idx + 1
        #遍历
        for k in targets.keys():
            targets[k] = targets[k].to(configs.device, non_blocking=True)
        imgs = imgs.to(configs.device, non_blocking=True).float()
        outputs = model(imgs) #训练
        total_loss, loss_stats = criterion(outputs, targets) #计算loss
