...
class PoseResNet(nn.Module):
    ...
    def __init__(self, block, layers,heads,  head_conv, **kwargs):
        ...
        for fpn_idx, fpn_c in enumerate(fpn_channels): #[256,128,64]  heads = [ 3 2 2 1 3]
            for head in sorted(self.heads):#  [3,2,2,1,3]  -> [1 2 2 3 3]
                num_output = self.heads[head]# 2,2,2,1,1
                if head_conv > 0: # head_conv = 64
                    fc = nn.Sequential(
                        nn.Conv2d(fpn_c, head_conv, kernel_size=3, padding=1, bias=True),#conv2d(256,64,3,1) conv2d(256,2,)
                        nn.ReLU(inplace=True),
                        nn.Conv2d(head_conv, num_output, kernel_size=1, stride=1, padding=0))#conv2d(64,2,1,1,0)
                else:
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        #通道数x1 or通道数x4 步数！= 1
        #但是步长=1了，所以不经过这一行
        if stride != 1 or self.inplanes != planes * block.expansion:  #  初始的inplanes = 64 , 64*basicblock.expansion = 1  相等，所以第一次不用downsample
    ...
    def forward(self, x):
        ...
        hm_h, hm_w = input_h // 4, input_w // 4 # Python中两个斜杠即双斜杠（//）表示地板除，即先做除法（/），然后向下取整（floor）
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        for head in self.heads:
            temp_outs = []
            for fpn_idx, fdn_input in enumerate([up_level2, up_level3, up_level4]):#遍历
    ...
    def apply_kfpn(self, outs):
        outs = torch.cat([out.unsqueeze(-1) for out in outs], dim=-1) #如果dim为负，则将会被转化dim+input.dim()+1 ，就是input，dim，增加一个inputdim维度，
    ...
    def init_weights(self, num_layers, pretrained=True):
        if pretrained:
            ...
            for fpn_idx in [0, 1, 2]:  # 3 FPN layers
                for head in self.heads: #[3 2 2 1 3]
                    final_layer = self.__getattr__('fpn{}_{}'.format(fpn_idx, head)) #fpn{}_{}:0 3
...
# test = get_pose_net(18,[3,2,2,1,3],64,True)
# print(test)